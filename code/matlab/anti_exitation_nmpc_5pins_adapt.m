%% anti_exitation_nmpc_5pins_adapt.m
...
if P.exp.runAblation
    ...
    save_ablation_results(results, P);
    fprintf('\nSaved ablation results. Run visualize_anti_exitation_5pins_adapt.m to regenerate figures without rerunning NMPC.\n');
else
...
function P = default_params()
    ...
    P.ltr.clipForPlot = 1.00;         % display boundary for rollover cases
    P.ltr.rollover = 1.00;            % |LTR| >= 1 means rollover occurred
    ...
    P.traj.speedKmh = 80;             % km/h, highway speed limit for tank trucks
...
function cases = default_ablation_cases()
    ...
    cases = [c1, c2, c3, c4, c5, c6, c8, c0, c9, c10, c11, c13, c14];
end
...
function save_ablation_results(results, P)
    scriptPath = mfilename('fullpath');
    matlabDir = fileparts(scriptPath);
    outPath = fullfile(matlabDir, 'anti_exitation_nmpc_5pins_adapt_results.mat');
    save(outPath, 'results', 'P', '-v7.3');
    fprintf('Saved MAT result file: %s\n', outPath);
end
...
function plot_results(result, P)
    ...
    cla;
    plot(time, ltrPlot, 'LineWidth', 1.8); hold on;
    yline(P.ltr.rollover, 'k--', 'Rollover');
    yline(-P.ltr.rollover, 'k--');
    grid on; xlabel('t (s)'); ylabel('LTR_{est}');
    title(sprintf('Estimated LTR, rollover at |LTR| = %.1f', P.ltr.rollover));
    legend('Displayed LTR','Location','best');
end

function plot_ablation_results(results, P)
    plot_independent_dlc_condition_summary(P);
    plot_segmentwise_statistics_only(results, P);
    return;
...
function plot_independent_dlc_condition_summary(P)
    phases = P.traj.dlcPhases;
    nSeg = size(phases, 1);
    speedKmh = getfield_with_default(P.traj, 'speedKmh', 80);
    D = P.traj.D;

    fig = figure('Name','Independent 5-pins DLC conditions', ...
        'Color','w','Position',[80 80 1220 560]);
    ...
    for iseg = 1:nSeg
        ...
        Tback = phases(iseg, 3);
        Tgap = phases(iseg, 4);
        tt = linspace(0, Tgo + Thold + Tback + Tgap, 240);
        yy = zeros(size(tt));
        for k = 1:numel(tt)
            t = tt(k);
            if t <= Tgo
                s = t / Tgo;
                yy(k) = D * (10*s^3 - 15*s^4 + 6*s^5);
            elseif t <= Tgo + Thold
                yy(k) = D;
            elseif t <= Tgo + Thold + Tback
                s = (t - Tgo - Thold) / Tback;
                yy(k) = D * (1 - (10*s^3 - 15*s^4 + 6*s^5));
            else
                yy(k) = 0;
            end
        end
        plot(tt, yy + (iseg-1)*0.12, 'LineWidth', 1.8, ...
            'Color', colors(iseg,:), 'DisplayName', sprintf('DLC%d', iseg));
    end
    xlabel('Time within one independent DLC (s)');
    ylabel('Lateral reference y (m), offset for display');
    title('Five independent double-lane-change references');
    legend('Location','northeastoutside');

    nexttile;
    axis off;
    title('Condition definition');
    linesText = strings(nSeg + 5, 1);
    linesText(1) = sprintf('Speed: %.0f km/h', speedKmh);
    linesText(2) = sprintf('Lateral displacement: %.1f m', D);
    linesText(3) = 'Each DLC is evaluated independently.';
    linesText(4) = '|LTR| = 1 means rollover occurred.';
    linesText(5) = 'DLC     outward LC     return LC';
    for iseg = 1:nSeg
        linesText(iseg+5) = sprintf('DLC%d       %5.2f s       %5.2f s', ...
            iseg, phases(iseg,1), phases(iseg,3));
    end
    text(0.02, 0.92, strjoin(linesText, newline), ...
        'Units','normalized', 'FontName','Consolas', 'FontSize', 12, ...
        'VerticalAlignment','top');

    sgtitle('5-pins validation conditions');
    save_paper_figure(fig, 'fivepins_conditions_generated.png');
end

function plot_segmentwise_statistics_only(results, P)
    if isempty(results) || isempty(results{1}.metrics.segmentRmsError)
        warning('No segment-wise metrics are available.');
        return;
    end

    allNames = strings(numel(results), 1);
    for i = 1:numel(results)
        allNames(i) = string(results{i}.name);
    end
    [idxPlot, labels] = select_representative_cases(allNames);

    segNames = results{1}.metrics.segmentNames;
    nSeg = numel(segNames);
    nPlot = numel(idxPlot);
    segRms = zeros(nSeg, nPlot);
    segPeak = zeros(nSeg, nPlot);
    segLTR = zeros(nSeg, nPlot);
    for j = 1:nPlot
        i = idxPlot(j);
        segRms(:,j) = results{i}.metrics.segmentRmsError(:);
        segPeak(:,j) = results{i}.metrics.segmentPeakAbsError(:);
        segLTR(:,j) = results{i}.metrics.segmentPeakAbsLTR(:);
    end

    rollover = getfield_with_default(P.ltr, 'rollover', 1.0);
    segLTRDisplay = min(segLTR, rollover);

    fig = figure('Name','Segment-wise statistics for independent DLCs', ...
        'Color','w','Position',[90 70 1280 860]);
    ...
    grid on; box on;
    ylabel('RMS error (m)');
    title('Tracking accuracy by independent DLC condition');
    ...
    grid on; box on;
    ylabel('Peak |error| (m)');
    title('Peak tracking error by independent DLC condition');
    set(ax2, 'XTick', 1:nSeg, 'XTickLabel', segNames);

    ax3 = nexttile;
    bar(segLTRDisplay, 'LineWidth', 0.8);
    hold on;
    yline(rollover, 'k--', 'Rollover', ...
        'LineWidth', 1.4, ...
        'LabelHorizontalAlignment','left', ...
        'LabelVerticalAlignment','bottom');
    grid on; box on;
    ylabel('Peak |LTR| at rollover boundary');
    xlabel('Independent DLC condition');
    title('Rollover-risk proxy by independent DLC condition');
...
function [idxPlot, labels] = select_representative_cases(names)
    wanted = [
        "Baseline: tracking only"
        "+ fixed Qband full"
        "+ preview adaptive Qband full"
        "+ feedback adaptive Qband full"
        "+ preview adaptive + band soft"];
    labelWanted = [
        "Baseline"
        "Fixed Q-band"
        "Preview Q-band"
    ...
    for k = 1:numel(wanted)
        idx = find(names == wanted(k), 1, 'first');
        if ~isempty(idx)
            idxPlot(end+1) = idx; %#ok<AGROW>
            labels(end+1,1) = labelWanted(k); %#ok<AGROW>
        end
    end
    if isempty(idxPlot)
        idxPlot = 1:min(numel(names), 5);
        labels = names(idxPlot);
    end
end
...
function save_paper_figure(fig, fileName)
    try
        ...
        warning('Could not export paper figure "%s": %s', fileName, ME.message);
    end
end
